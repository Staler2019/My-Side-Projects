...
int main(void) {
    int number[LEN] = {1, 3, 4, 5, 9, 2, 1, 2353, 214, 324};
...
void shakerSort(int* number, int len, int (*compar)(int, int)) {
    int left, right;
    for (left = 0, right = len - 1; left < right; right = bubbleL2R(number, left, right, compar),
...
int bubbleL2R(int* arr, int left, int right, int (*compar)(int, int)) {
    int i, swapped;
    for (i = left, swapped = left; i < right; i++)
...
int bubbleR2L(int* arr, int left, int right, int (*compar)(int, int)) {
    int i, swapped;
    for (i = right, swapped = right; i > left; i--)
