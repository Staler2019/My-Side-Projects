...
/*- 函式 -*/
vector<bool> x_or(vector<bool> a, const vector<bool> b) {
    vector<bool> result, base;
    if (b.size() > a.size()) {
        base = a;
        result = b;
    } else {
    ...
    for (int i = result.size() - 1, j = base.size() - 1; j >= 0; i--, j--) {
        // result[i] ^= base[j];
        result[i] = bool(result[i]) ^ bool(base[j]);
    }
...
vector<bool> remainder(vector<bool> a, vector<bool> g) {
    int counter = g.size();
    for (int i = 0;;) {  // i means indicator of
                         // "a" bits string
        vector<bool> tmp_a;

        for (int j = 0; j < counter && i < a.size(); i++, j++) tmp_a.pb(a[i]);
        if (tmp_a.size() != g.size()) return tmp_a;
        // function
        tmp_a = x_or(tmp_a, g);
        counter = g.size() - tmp_a.size();
    }
}

vector<bool> bool_cast(string s) {
    vector<bool> result;
...
int InSolving() {
    ...
    cin >> b;
    while (1) {
        vector<bool> b_a = bool_cast(a), b_b = bool_cast(b);

        for (int i = 0; i < a.size() - 1; i++) {  // shift
            b_b.push_back(0);
        }

        vector<bool> result = remainder(b_b, b_a);
